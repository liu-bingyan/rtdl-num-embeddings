...
# for folders in exp with the name [mlp-lr, mlp-t, mlp-t-lr, resnet-lr, resnet-t, resnet-t-lr, transformer-lr, transformer-t, transformer-t-lr], called $model
#      for each subfolder in $model, called $dataset,
#         open 0_evaluation/0/report.json, called report_file
#         read report_file, retrive config.model,
#         write config.model as json value under the attribute 
#           $dataset.$model in to rtdl_best_params.json
#             

# def format_param(param_dict,basemodel):
#     param_tab = {}
#     param_tab['num_bins'] = param_dict['d_num_embedding']
#     if basemodel == 'mlp':
#         param_tab['hidden_dim'] = param_dict[basemodel]["d_layers"]
#         param_tab['dropout'] = param_dict[basemodel]['dropout']
#     if basemodel == 'resnet':
#         param_tab = param_dict[basemodel]
#     if basemodel == 'transformer':
#         param_tab['num_heads'] = param_dict[basemodel]['num_heads']
#         param_tab['num_layers'] = param_dict[basemodel]['num_layers']
#         param_tab['hidden_dim'] = param_dict[basemodel]['d_layers']
#         param_tab['dropout'] = param_dict[basemodel]['dropout']
